 ...
 */
function heightAt(recipe: MaterialRecipe, u: number, v: number, seed: number): number {
  const s = recipe.detailScale;
  switch (recipe.family) {
    case 'metal_brushed':
      // Strongly anisotropic: fine streaks along U, almost no variation along V.
      return fbm(u * s, v * 2, seed, { octaves: 3, period: 64 }) * 0.7 + valueNoise(u * s * 3, v, 128, seed + 5) * 0.3;
    case 'asphalt': {
    ...
    case 'fabric': {
      const warp = Math.sin(u * s * Math.PI * 2) * 0.5 + 0.5;
      const weft = Math.sin(v * s * Math.PI * 2) * 0.5 + 0.5;
      return Math.max(warp, weft) * 0.7 + fbm(u * s, v * s, seed, { octaves: 2, period: 32 }) * 0.3;
    }
    case 'hair':
      return fbm(u * s * 0.15, v * s * 6, seed, { octaves: 3, period: 48 });
    case 'skin':
      // Fine pore noise over slow subdermal variation.
      return fbm(u * s * 4, v * s * 4, seed, { octaves: 2, period: 64 }) * 0.35 + fbm(u * 5, v * 5, seed + 11, { octaves: 3, period: 16 }) * 0.65;
    case 'rock':
...
function albedoAt(recipe: MaterialRecipe, height: number, wear: number, base: Rgb, secondary: Rgb): Rgb {
  ...
  switch (recipe.family) {
    ...
    case 'skin': {
      const flush = hslToRgb({ h: hsl.h - 4 + height * 8, s: hsl.s * (0.9 + height * 0.2), l: hsl.l * (0.94 + height * 0.12) });
      return flush;
    }
    case 'emissive_panel':
...
export function generateTextureSet(recipe: MaterialRecipe, options: TextureSetOptions): TextureSet {
  ...
  for (let y = 0; y < size; y += 1) {
    for (let x = 0; x < size; x += 1) {
      ...
      heights[index] = heightAt(recipe, u, v, seed);
  ...
  for (let y = 0; y < size; y += 1) {
    for (let x = 0; x < size; x += 1) {
      ...
      // normalised against a 64-pixel reference. Without this a 512px map bakes
      // normals eight times too strong and every surface looks crinkled.
      const strength = recipe.relief * (size / 64);
      ...
      const roughness = Math.max(0.02, Math.min(1, recipe.roughness + worn * 0.35 + (height - 0.5) * 0.12));
      const metallic =
